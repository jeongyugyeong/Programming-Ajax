...
function packageHint() { // 함수를 선언한다
    ...
    if (package === 'monthly') { //월 단위 패키지를 선택했으면
        elPackageHint.innerHTML = '1년 패키지를 사용하면 10불을 절약할 수 있습니다!'; //이 메시지를 보여준다
    } else { //그렇지 않으면
    ...
    //이벤트 리스너를 생성한다. submit 이벤트가 발생하면 checkTerms() 함수를 호출하고,
    //change 이벤트가 발생하면 packageHint() 함수를 호출한다
